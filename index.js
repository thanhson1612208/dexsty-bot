...
client.on("messageCreate", async (message) => {
    ...
    if (message.channel.id === CHAT_CHUNG_ID) {
        ...
        if (botCommands.some(cmd => message.content.startsWith(cmd))) {
            return message.reply("❌ **Thông báo:** Bot không được dùng ở kênh này. Sang kênh lệnh riêng nhé!")
    ...
    if (message.content === '!menu') {
        const row1 = new ActionRowBuilder().addComponents(
            new StringSelectMenuBuilder().setCustomId('menu_gp').setPlaceholder('🎮 Chọn Game Pass / Robux').addOptions([
        ...
        const row3 = new ActionRowBuilder().addComponents(
            new StringSelectMenuBuilder().setCustomId('menu_p2').setPlaceholder('🔥 Trái Vĩnh Viễn (Trang 2)').addOptions([
                ...
                { label: 'Perm Magma', value: 'magma', description: '200K' }, { label: 'Perm Love', value: 'love', description: '244K' },
                { label: 'Perm Buddha', value: 'buddha', description: '239K' }, { label: 'Perm Portal', value: 'portal', description: '252K' },
                { label: 'Perm Rumble', value: 'rumble', description: '264K' }, { label: 'Perm Phoenix', value: 'phoenix', description: '278K' },
                { label: 'Perm Sound', value: 'sound', description: '291K' }, { label: 'Perm Blizzard', value: 'blizzard', description: '291K' },
                ...
                { label: 'Perm Control', value: 'control', description: '332K' }, { label: 'Perm Spirit', value: 'spirit', description: '332K' },
                { label: 'Perm T-Rex', value: 'trex', description: '341K' }, { label: 'Perm Mammoth', value: 'mammoth', description: '348K' },
                { label: 'Perm Leopard', value: 'leopard', description: '425K' }, { label: 'Perm Kitsune', value: 'kitsune', description: '574K' },
...
client.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.isButton()) {
        ...
        if (parts[0] === 'paid') {
            await interaction.update({ content: "⏳ Hãy gửi **Ảnh Bill** vào đây để Admin duyệt!", components: [], files: [] });
        ...
        if (parts[0] === 'approve' || parts[0] === 'deny') {
            ...
            if (parts[0] === 'approve') {
                await interaction.update({ content: `✅ Đã duyệt`, components: [] });
                if (targetUser) targetUser.send("✅ Đơn của bạn đã được duyệt!");
            } else {
                await interaction.update({ content: `❌ Đã từ chối`, components: [] });
